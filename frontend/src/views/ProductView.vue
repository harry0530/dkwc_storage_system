...
const onBomFileChange = (e) => {
  bomUploadFile.value = e.target.files?.[0] || null;
};
...
const uploadBomBlocksExcel = async () => {
  if (!bomUploadFile.value) return;

  const buildUploadForm = (duplicateAction) => {
    const form = new FormData();
    form.append("file", bomUploadFile.value);
    form.append("duplicate_action", duplicateAction);
    return form;
  };
  ...
  let res;
  try {
    res = await api.post("/bom/import-blocks", buildUploadForm("prompt"), {
      headers: { "Content-Type": "multipart/form-data" }
    });
  } catch (err) {
    const detail = err?.response?.data?.detail;
    if (err?.response?.status === 409 && detail?.duplicate_count) {
      const previewCodes = Array.isArray(detail.duplicate_codes) && detail.duplicate_codes.length
        ? `\n중복 완제품 예시: ${detail.duplicate_codes.join(", ")}`
        : "";
      const overwrite = window.confirm(
        `이미 BOM이 등록된 완제품이 ${detail.duplicate_count}건 있습니다.${previewCodes}\n\n확인: 기존 BOM 덮어쓰기\n취소: 중복 완제품 스킵`
      );
      const duplicateAction = overwrite ? "overwrite" : "skip";
      res = await api.post("/bom/import-blocks", buildUploadForm(duplicateAction), {
        headers: { "Content-Type": "multipart/form-data" }
      });
    } else {
      alert(detail?.message || detail || "BOM 업로드 중 오류가 발생했습니다.");
      return;
    }
  }

  bomUploadFile.value = null;
  await loadData();
  const blocksTotal = Number(res?.data?.blocks_total || 0);
  const createdFinished = Number(res?.data?.created_finished || 0);
  const updatedFinished = Number(res?.data?.updated_finished || 0);
  const overwrittenBlocks = Number(res?.data?.overwritten_blocks || 0);
  const skippedBlocks = Number(res?.data?.skipped_blocks || 0);
  const bomRowsTotal = Number(res?.data?.bom_rows_total || 0);
  alert(`BOM 업로드 완료 (블록 ${blocksTotal}건, 완제품 신규 ${createdFinished}건, 완제품 갱신 ${updatedFinished}건, BOM 행 ${bomRowsTotal}건, 덮어쓰기 ${overwrittenBlocks}건, 스킵 ${skippedBlocks}건)`);
};
  ...
  <div>
    ...
    <div class="panel mb-6">
      ...
      <div class="p-3 space-y-3">
        <div class="flex gap-2 items-center flex-wrap">
          <span class="text-sm text-slate-500 min-w-[88px]">완제품 업로드</span>
        ...
        <div class="flex gap-2 items-center flex-wrap">
          <span class="text-sm text-slate-500 min-w-[88px]">BOM 업로드</span>
          <input type="file" @change="onBomFileChange" class="input w-72" />
          <button @click="uploadBomBlocksExcel" class="btn btn-primary">업로드</button>
